...
describe('error reports', () => {
  ...
  it('accepts a report from the old editor too', async () => {
    // The positional payload only the old editor sends, accepted as an opaque
    // object so a report arriving mid-cutover is not rejected.
  ...
  it('mails the report with the circuit attached', async () => {
    ...
    // The most useful part of a report, and the one that would make the body
    // unreadable inline.
  ...
  it('caps a field rather than storing whatever arrives', async () => {
    ...
    // An unauthenticated write of attacker-chosen text; the bound is all that
    // stands between it and a log nobody can read.
  ...
  it('still answers when there is nowhere to mail it', async () => {
    const fresh = await startE2eApp();
    try {
      // Logging always works; a mailbox is opt-in.

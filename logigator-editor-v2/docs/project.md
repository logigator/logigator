...
| `claimPendingCut()`        | Returns the `ActionContainer` recorded by a tentative scissor cut (SELECT_EXACT) and clears the pending state. Returns `null` if no cut is pending. Called by `SelectionMoveSession.onEnd()` (folds the cut into the move undo entry) and by `ClipboardService._applyDelete()` (folds the cut into the delete undo entry). See `wires.md` § _Tentative cut + commit on move_. |

...
function makeProject() {
  ...
  } as unknown as Project & { selectionManager: { commit: ReturnType<typeof vi.fn> } };
}

describe('PanSession', () => {
  it('pans by the screen-space delta between successive moves', () => {
    const project = makeProject();
    const session = new PanSession(project, new Point(100, 100), new Point(0, 0));
    ...
    expect((project.pan as ReturnType<typeof vi.fn>).mock.calls[0][0]).toMatchObject({
      x: 10,
      y: -5
    });

    session.onMove(makePanEvent(130, 90));
    expect((project.pan as ReturnType<typeof vi.fn>).mock.calls[1][0]).toMatchObject({
      x: 20,
      y: -5
    });
  });

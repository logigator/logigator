...
export class ButtonRenderer extends BaseRenderer {
	render(): SVGGElement {
		let path = `M 0,0 h ${this.gridSize} v ${
			this.gridSize
		} H 0 V 0 M ${this.scaled(3)},${this.scaled(3)} H ${
			this.gridSize - this.scaled(3)
		} V ${this.gridSize - this.scaled(3)} H ${this.scaled(3)} V ${this.scaled(
			3
		)}`;

...
- `logigator-core/src/` — the rendering-free half of the circuit code: `model/` (the shapes a
  ...
  `CircuitFileV0/V1` envelopes, errors, the structural validator, the `.lgix` container),
  ...
  relative escape fail too. The migrator + migration chain and the server-facing
  `parseCircuitDocument` pipeline land in the later stages of Phase 1 of
  `plans/backend-rewrite.md`.

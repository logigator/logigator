...
- `inspection/` — Live component inspection during simulation. Tapping an inspectable component (its config declares an `inspection` factory returning a `ComponentInspection` — the option/action-analog model in `components/`) opens its view: a floating `@logigator/ui` window on desktop, the shared non-modal bottom sheet on compact, re-homed live on breakpoint flips. Data is pull-based via `SimulationService.frame$` (per applied snapshot). ROM is the first inspectable type (read-only hex editor, addressed word highlighted). Doc: `inspection.md`.

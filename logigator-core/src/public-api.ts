 ...
 *
 * Consumed buildless through the workspace tsconfig `paths` mapping: every
 * consumer's bundler compiles this source directly. The package stays free of
 * runtime dependencies and of any framework or renderer import, enforced by
 * `eslint.config.mjs` and by a standalone `tsc --noEmit` over the package
 * alone.
 *

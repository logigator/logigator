...
Direction commits diverge by branch: a **placed** component routes through `project.requestSelectionRotation(steps)` (midpoint pivot, collision handling, shared undo entry with the rotate buttons); a **placement ghost** writes the sticky per-type direction on `WorkModeService` (`setPlacementDirection`) — as does a rotate request aimed at the armed ghost (`rotatePlacementDirection`, see [`rendering.md`](rendering.md)) — which every fresh ghost of that type starts from and the standing preview follows.

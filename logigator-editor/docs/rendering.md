...
- **Click count** — each primary press carries its position in a run of quick presses: the DOM's own rule (`MouseEvent.detail` — within 500 ms and 6 px of the previous press, otherwise a new run), counted from `event.timeStamp` on the press itself, so a tool can tell a double click from two deliberate single clicks. Only `WireTool` acts on it today (see `work-mode.md` § _Tools_).
...
- **`PanTool`** — opens a `PanSession`; a tap single-selects (the session's default tap action, or toggles that element in or out of the selection with the additive modifier held). With the `dragSelectionInPanMode` setting on (the default), a press that grabs the committed selection opens a `SelectionMoveSession` instead — the select tool's move gesture — so a selection made by a tap can be dragged without leaving the tool; every other press pans.
...
- **`WireTool`** — a press inside a selected component's body opens a `SelectionMoveSession` instead of a `WireToolSession` (a wire run from there would collide with that body and be discarded), so a selection can be moved without leaving the tool; elsewhere it opens a `WireToolSession`; owns the tap action (port negation through the undo stack, else `project.topology.toggleConnectionAt`, which returns the action it recorded, else a plain select click through `SelectionManager.commit`, or a toggle with the additive modifier held, which outranks both circuit actions) and the hover previews: the negation bubble over a port (translucent for the bubble a tap would add; opaque in the theme's `invalid` color over a bubble a tap would remove), or the connection ghost over a toggleable junction (`project.topology.connectionToggleKindAt` dry-runs the join plan so non-toggleable T-junctions show nothing). The previews survive a press and hide only when the gesture becomes a drag (`WireToolSession` hides them on its first real move); after a tap they re-derive in place. A press whose `clickCount` continues the previous tap takes that tap back — its action out of the history (`ActionManager.retract`), the selection it ran against back onto the board — and swallows its own — see `work-mode.md` § _Tools_ for why.
- **`SelectTool`** — one instance per marquee flavor (SELECT, SELECT_EXACT): a press inside the committed selection's grab zone opens a `SelectionMoveSession` (which runs the click the press turned out to be if it never moved: narrow to the pressed element, or toggle it with the additive modifier — see `SelectionManager.clickInSelection`), anywhere else a `SelectRectSession` with the flavor, the live hold-to-scissor key state and the additive one.
- **`PlacementTool`** — holds the palette selection (`setConfig`, fed through the router's `componentToPlace` setter) and follows the cursor with a `PlacementGhost` — the same grid-snapped, collision-tinted ghost the placement session shows, so pressing hands off seamlessly (`onSessionStart` destroys the hover ghost as the session's replaces it). `down` ensures a cloud custom master's circuit is loaded before opening the `ComponentPlacementSession`, guarded by the host's gesture stamp. `rotate` (the `BoardTool` hook) steps the sticky placement direction and turns the live preview — the hover ghost follows a direction the settings panel changed mid-hover the same way, on the next pointer move.
...
- **Lifecycle** — created lazily on the first lease (`autoDetectRenderer`, `webgl` → `canvas` ladder), destroyed when the last lease releases. In practice the board holds a lease for its whole lifetime, so the renderer lives as long as a board is mounted; `available` flips true once it boots.
- **WebGPU is deliberately disabled** — pixi 8.19's WebGPU backend needed three patches to work here at all and still lost the GPU device on AMD/D3D12. The full findings — root causes, the patch hunks, remaining unknowns, and the re-enablement checklist — live in `docs/webgpu.md`. One consequence remains active in code: `BoardSnapshotService` honors `SnapshotOptions.coverageBoost` only on the WebGL renderer, because WebGL readbacks come back darker than WebGPU's (see the doc's Issue 4).
- **Multi-canvas** — the WebGL renderer is created with `multiView` (an off-DOM master canvas sized to the largest target, blitted to each target canvas per render — one extra copy per frame).
...
**Paste placement** bypasses the work-mode switch: the router opens a `PastePlacementSession` in hover mode (`isDragging = false`) on a project's `pasteRequest$`. The router positions the group first (`_positionPasteGroup`): the fresh instances carry the copied coordinates, and the group is shifted by whole grid units (components sit on the lattice, wires on its half-steps) to sit centred on the cursor, or on the middle of the grid view (`viewport.gridView`) when there is no cursor — touch never sets one and leaving the canvas clears it. The resting cursor is recorded in canvas-local pixels and mapped through `canvasToGrid` at paste time: pans and the zoom controls move the camera with no pointer move behind them, so a stored grid coordinate would go stale. In hover mode, the ghosts do not follow the cursor — they wait where the router put them. The session receives the next press through `DragSession.onDown`: a click inside the group's padded rect — the one the ghosts wear, so gaps between the pasted elements grab too, exactly as a committed selection does through `SelectionManager.isGrabbedAt` — calls `beginDrag()` and subsequent moves drag the group on a grid-snapped cursor; a click outside that rect returns `false`, asking the router to cancel the session (the fresh instances are destroyed).
...
**`PastePlacementSession`** — opened by the router on a project's `pasteRequest$` when the user invokes paste. Receives pre-deserialized `Component[]` and `Wire[]` (fresh instances with new IDs, already positioned by the router at the cursor or the view centre). Elements are added to `_dragLayer` with `selected = true` — the ghosts wear the selection look, which carries over seamlessly when `select()` keeps them selected on commit. Two-phase interaction:

1. **Hover phase** (`isDragging = false`) — elements sit where the router put them. `onMove` is a no-op. A press inside the group's rect (`containsPoint`, the padded content bounds the rect visual draws) begins the drag; a press outside it cancels the paste.

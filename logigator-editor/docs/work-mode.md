...
> **The wire tool (WIRE_TOOL)**: a drag draws wires, except inside a selected component's body, where it drags that selection instead — a run started there collides with the body and is discarded anyway (`SelectionMoveSession`, opened on the selection's own tap, so a modified press toggles as it does elsewhere). Everywhere a run can start the wire tool keeps drawing: from a port, from a wire's interior, from a junction. A press that never leaves its starting grid step is a **tap**, which `WireToolSession` reports back to its tool (`WireTool`, via `onTap`): a port within 0.5 grid units toggles its negation bubble (undoably, via `TogglePortNegationAction`) — this is what the hover ghost previews — otherwise the nearest half-grid point toggles the wire connection there (`project.topology.toggleConnectionAt`: join/split, a graceful no-op when neither applies, returning the action it recorded). If a port and a junction are both in reach, the port wins, matching the ghost. A tap that would do **neither** falls through to the select click (`SelectionManager.commit`, the same zero-rect path the hand tool's tap takes): it selects the smallest element under the cursor, or clears the selection over empty canvas — so a component or wire can be picked up (to delete, copy or nudge it, or to read the selection count) without leaving the wire tool. A selection is not a history entry; the two circuit actions outrank it, so a tap that can change the circuit always does and the hover ghost keeps meaning "this tap will change the circuit" (no ghost now means "this tap will select"). A modified click toggles instead, ahead of both. Drag-to-move stays out: the drag is the wire.
>
> **The additive modifier** (`SELECT_ADDITIVE`, a hold-style binding — Ctrl, or ⌘ on Apple platforms, where a bare Ctrl is the system's secondary click) makes a click _toggle_ the element under it in `SelectionManager.commit`'s click path: in when it was out, out when it was in, in every tool that can select. It outranks both circuit actions, so a click that is building a selection never negates a port or toggles a junction on its way; a click that hits nothing leaves the selection alone (a plain click replaces it). A press on the selection is unaffected — it still starts the move, and letting go without moving runs the click instead of nothing (`SelectionMoveSession`'s `onTap`, the same fallback `PanSession` uses, both through `SelectionManager.clickInSelection`): the element under the press becomes the whole selection, which is how a click narrows a multi-selection to one of its members, or toggles it with the modifier — the same whether that element sits inside the committed marquee or beside it. A press that landed on the gap inside a marquee leaves the selection alone, so aiming beside a small element never clears everything. There are no separate wire-connection or port-negation modes.
>
> **Double clicks**: people reach for a wire by double-clicking the port they start from, and the first click's footprint — a bubble left standing, or a selection made where the wire is about to leave from — reads as a mark nobody asked for. A press that continues the previous tap into a double click (`clickCount`, the controller's DOM-style count) therefore **takes the first tap back** (its action out of the history via `ActionManager.retract`, plus the selection it ran against, restored with `select()` + `freezeGrabRect` so it comes back without a re-derived grab rect — a junction toggle can consume a selected wire and its undo re-adds the wires unselected) and **swallows its own tap**, so the pair cancels out — exactly what a double click already did when both halves stayed taps. A tap whose action the history has moved past leaves the second press alone, and a press that lands elsewhere or after the window is a fresh click: the collapse only ever pairs clicks that really are one double click.
...
- **`PAN`** (`PanTool`) — opens a `PanSession`. With the `dragSelectionInPanMode` setting on (the default), a press that grabs the committed selection opens a `SelectionMoveSession` instead, so a selection made by a tap can be dragged without leaving the hand tool; the setting off makes every press pan. Both paths need the selection `isGrabbedAt` the press, so panning off the selection — empty canvas, or an element that is not selected — is unaffected either way; the grab zone is the same one the select tool uses, the drawn marquee's interior included.
...
- **`WIRE_TOOL`** (`WireTool`) — on `pointerdown`, snaps to half-grid (wire endpoints sit between grid cells) and checks the press's `clickCount` for a double-click collapse (see the wire-tool note above). During `pointermove`, determines drag direction on first movement and updates two orthogonal `Wire` objects (one horizontal, one vertical) to create an L-shaped preview. On `pointerup`, the session adds the drawn wires (integrated: splits/merges applied with live instances) and records via `ActionManager.register` — unless the pointer never left its starting grid step, in which case the session fires its `onTap` callback and the tool runs the tap action (port negation, else connection toggle, else a select click; see the wire-tool note above). While no pointer is pressed, `hover` previews the tap: the negation ghost over negatable ports, the connection ghost over toggleable junctions (`project.topology.connectionToggleKindAt` — 'split' shows the dot a tap would create, 'join' tints the existing dot red for removal; a dry-run of the join plan keeps T-junctions ghost-free). The previews survive the press itself and hide only when the gesture becomes a drag; after a tap the ghost re-derives in place (split ⇄ join).
...
BoardComponent  →  effect()  →  WorkModeRouter.setMode / .componentToPlace
                              ...
                              └── down()  →  tools.get(mode).down()
                                  ...
                                  │               |  SelectionManager.commit (plain click)
                                  ├── EraseTool             →  EraseSession
                                  ├── PanTool               →  SelectionMoveSession (a press grabbing the
                                  │                             selection, with dragSelectionInPanMode)
                                  ├── SimulationTool        →  PanSession

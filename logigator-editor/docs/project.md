...
| Member                              | Description                                                                                                                                                                                                                                                                                                                            |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `commit(rect, mode, additive?)`     | Dispatch: zero-area rect → single-click hit test; non-zero rect → rectangle selection. Clears the previous selection first, unless `additive` (the hold-style modifier) is set — a click then toggles the element under it and a marquee joins what it touches, leaving a live scissor cut retracted but the selection standing.       |
...
| `clickInSelection(point, additive)` | The click a press on the selection that never moved turned out to be: narrows the selection to the element under the point, or toggles it with `additive`; a click that lands on the gap inside a marquee leaves the selection alone. Called by `SelectionMoveSession`'s tap fallback, which the select, hand and wire tools all open. |
| `isGrabbedAt(gridPoint)`            | Whether a press grabs the selection (starts a move): the drawn marquee where it covers the point, else any selected element's own bounds — found through the spatial index, not by walking the selection.                                                                                                                              |
| `selectionVersion`                  | A counter that changes whenever the selection does, so provisional state elsewhere (the wire tool's double-click take-back) can tell whether what it recorded still stands.                                                                                                                                                            |
...
1. Calls `clear()` to drop the old selection — unless the commit is `additive`, which retracts a live scissor cut and keeps the selection instead.
...
**Single click** (`rect.width === 0 && rect.height === 0`), also the entry point for
`clickInSelection(point, additive)` — what a press on the selection that never
moved runs, for all three tools:

1. With `additive`, toggles the element under the point (no-op when it lands on
   nothing) and returns. Otherwise clears — before resolving the hit, since the
   clear may retract a live cut and replace the wires it cut.

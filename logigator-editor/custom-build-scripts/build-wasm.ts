...
if (fs.existsSync(path.join(source, 'logigator-simulation.js')) && fs.existsSync(path.join(source, 'logigator-simulation.wasm'))) {
	fs.readdirSync(dest)
		...
		.forEach(file => fs.unlinkSync(path.join(dest, file)));

	fs.copyFileSync(path.join(source, 'logigator-simulation.wasm'), path.join(dest, `logigator-simulation.${timestamp}.wasm`));

	const simulationJs = fs.readFileSync(path.join(source, 'logigator-simulation.js'))
		.toString()
		.replace(/var wasmBinaryFile ?= ?["'](.*?)["'];/, `var wasmBinaryFile = \'assets/wasm/logigator-simulation.${timestamp}.wasm\';`);

	fs.writeFileSync(path.join(dest, 'logigator-simulation.js'), simulationJs);
} else {
	process.exit(1);
}

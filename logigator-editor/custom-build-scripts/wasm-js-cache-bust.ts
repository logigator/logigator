...
const jsFilePath = join(editorDist, 'assets', 'wasm', 'logigator-simulation.js');

async function run() {
	const jsFile = await readFile(jsFilePath);
	const jsHash = createHash('md5').update(jsFile).digest('base64url');
	await rename(jsFilePath, join(editorDist, 'assets', 'wasm', `logigator-simulation.${jsHash}.js`));

	const files = (await readdir(editorDist, { recursive: false }))
	...
	for (const filename of files) {
		...
		if (!content.includes('importScripts(\"assets/wasm/logigator-simulation.js\")')) {
			continue;
		}

...
export abstract class Elements {
	...
	public static clone(element: Element): Element {
		...
		if (element.endPos) out.endPos = element.endPos.clone();
		delete out.wireEnds;
	...
	public static equals(elem0: Element, elem1: Element): boolean {
		for (const k in elem0) {
			...
			if (elem0[k] && elem0[k].x !== undefined && elem0[k].y !== undefined) {
				...
				const v0 =
					elem0[k] === undefined || elem0[k] === null ? undefined : elem0[k];
				const v1 =
					elem1[k] === undefined || elem1[k] === null ? undefined : elem1[k];
	...
	public static genNewElement(
		...
		if (pos && endPos) Elements.correctPosOrder(pos, endPos);
		return {
			id: -1,
			typeId,
	...
	public static gen2Wires(
		_pos: PIXI.Point,
		_cornerPos: PIXI.Point,
		_endPos: PIXI.Point
	): { wire0: Element; wire1: Element } {
	...
	public static calcEndPos(
		element: Element,
		numInputs?: number,
		numOutputs?: number,
		rotation?: number
	): PIXI.Point {
		const elemSize = this.calcElemSize(
			element,
			numInputs,
			numOutputs,
			rotation
		...
		return new PIXI.Point(
			element.pos.x + elemSize.x,
			element.pos.y + elemSize.y
		);
	}

	public static calcElemSize(
		element: Element,
		numInputs?: number,
		numOutputs?: number,
		rotation?: number
		...
		rotation =
			rotation === undefined || rotation == null ? element.rotation : rotation;
		...
		if (rotation % 2 === 0) {
			return new PIXI.Point(
				elemType.width(elemToCalc),
				elemType.height(elemToCalc)
			);
		} else {
			return new PIXI.Point(
				elemType.height(elemToCalc),
	...
	public static calcPixelElementSize(element: Element): PIXI.Point {
		...
		return new PIXI.Point(
			gridSize.x * environment.gridPixelWidth,
			gridSize.y * environment.gridPixelWidth
		);
	}
	...
	public static mergeCheckedWiresVertical(
		wire0: Element,
		wire1: Element,
		newElem
	) {
	...
	public static mergeCheckedWiresHorizontal(
		wire0: Element,
		wire1: Element,
		newElem
	) {
	...
	public static wireEnds(element: Element): PIXI.Point[] {
		...
		if (element.wireEnds) return element.wireEnds.map((p) => p.clone());
		const out = this.wireEndsWithChanges(
			element,
			element.rotation,
			element.numInputs,
			new PIXI.Point()
	...
	public static wireEndsWithChanges(
		element: Element,
		rotation: number,
		numInputs: number,
		dif: PIXI.Point
		...
		if (element.typeId === ElementTypeId.WIRE) return [pos, endPos];
		const ignoreOutputs = Elements.elementType(element.typeId).ignoreOutputs;
		const out: PIXI.Point[] = new Array(
			numInputs + (ignoreOutputs ? 0 : element.numOutputs)
		);
		switch (rotation) {
			case 0:
				for (let i = 0; i < numInputs; i++)
					out[i] = new PIXI.Point(pos.x - 1, pos.y + i);
				if (ignoreOutputs) break;
	...
	public static wireEndsIfInOtherChunk(
		element: Element,
		rotation: number,
		numInputs: number,
		dif: PIXI.Point
		...
		if (
			(element.pos.x + dif.x) % 16 > 0 &&
			(element.pos.y + dif.y) % 16 > 0 &&
			(element.endPos.x + dif.x) % 16 < 15 &&
			(element.endPos.y + dif.y) % 16 < 15
		) {
			return [];
		} else {
	...
	public static allWireEnds(
		elements: Element[],
		startMap?: Map<number, Set<number>>
	...
	public static isHorizontalElem(elem: Element): boolean {
		return elem.typeId === ElementTypeId.WIRE
			? Elements.isHorizontal(elem)
			: elem.rotation % 2 === 0;
	}

	public static isSameDirection(elem0: Element, elem1: Element): boolean {
		return (
			Elements.isHorizontalElem(elem0) === Elements.isHorizontalElem(elem1)
		);
	}

export const enum WorkMode {
  // Hand tool: one pointer pans the board. The navigate-first default, so a
  // fresh board moves on first drag rather than mutating.
  PAN = 'pan',
  ...
  WIRE_TOOL = 'wireTool',
  SELECT = 'sel',
  // The select tool's scissor sub-state: wires are cut at the marquee edge on
  // commit.

...
export class ApiService {
	...
	public get<T>(url: string, errorConfig?: ApiErrorConfig): Observable<T> {
		return this.http.get<T>(environment.api + url).pipe(
			catchError(err => this.handleError(err, errorConfig))
		);
	}

	public post<T>(url: string, body: any, errorConfig?: ApiErrorConfig): Observable<T> {
		return this.http.post<T>(environment.api + url, body).pipe(
			catchError(err => this.handleError(err, errorConfig))
		);
	}

	public put<T>(url: string, body: any, errorConfig?: ApiErrorConfig): Observable<T> {
		return this.http.put<T>(environment.api + url, body).pipe(
			catchError(err => this.handleError(err, errorConfig))
		);
	}

	public patch<T>(url: string, body: any, errorConfig?: ApiErrorConfig): Observable<T> {
		return this.http.patch<T>(environment.api + url, body).pipe(
			catchError(err => this.handleError(err, errorConfig))
		);
	}

	public delete<T>(url: string, errorConfig?: ApiErrorConfig): Observable<T> {

...
export class StateCompilerService {
	...
	private initElemsOnLinksCache(identifier: number): void {
		...
		this._wireEndsOnLinksCache.set(
			identifier,
			new Map<number, WireEndOnComp[]>()
		);
	}
	...
	private async projectsToCompile(
		project: Project
	...
	private compileSingle(project: Project): void {
		...
		this._udcCache.set(
			project.id,
			this.calcCompiledComp(project.currState, unitElems)
		);
	...
	private compilerOuterIfNeeded(conPlugs: number[][], project: Project) {
		if (
			!ArrayHelper.array2dSame(
				conPlugs,
				this._udcCache.get(project.id).connectedPlugs
			)
		) {
	...
	private calcCompiledComp(
		...
		const linksOnWireEnds: WireEndLinksOnElem = new Map<
			Element,
			LinkOnWireEnd
		>();
	...
	private setAllLinks(
		...
		for (const element of unitElems.unitToElement.values()) {
			...
			for (const wireEndPos of Elements.wireEnds(element)) {
				wireEndIndex++;
				if (
					SimulationUnits.wireIdHasLink(linksOnWireEnds, element, wireEndIndex)
				) {
					continue;
				}
				linkId =
	...
	private setTunnelLink(
		...
		elem: Element,
		state: ProjectState,
		linksOnWireEnds: Map<Element, LinkOnWireEnd>,
		linkId: number,
		unitElems: UnitElementBidir,
		coveredPoints: PosOfElem[]
	) {
	...
	private setWireLink(
		...
		ArrayHelper.pushInMapArrayUnique(
			this._wiresOnLinksCache.get(this._currTypeId),
			linkId,
			elem
		);
	}
	...
	private setCompLink(
		...
		SimulationUnits.setInputOutput(
			unitElems.elementToUnit.get(elem),
			wireIndex,
			linkId
		);
		ArrayHelper.pushInMapArray(
		...
		for (const conPlugs of this._udcCache.get(elem.typeId).connectedPlugs) {
			if (conPlugs.includes(index)) {
				for (const wireEndIndex of conPlugs) {
					if (wireEndIndex === index) continue;
					this.setLinks(
						state,
						Elements.wireEnds(elem)[wireEndIndex],
						linksOnWireEnds,
						linkId,
						unitElems,
						compiledComp,
	...
	private projectUnits(
		...
		if (units.length === ElementTypeId.WIRE) return [];
		const linkMap = new Map<number, number>();

		if (outerUnit) {
			for (const [outer, inner] of compiledComp.plugsByIndex) {
				linkMap.set(
					SimulationUnits.concatIO(units[inner])[0],
					SimulationUnits.concatIO(outerUnit)[outer]
				);
				this._highestLinkId++;
			}
		}

		if (this._highestLinkId > 0) this._highestLinkId++;
		...
		for (const unit of units) {
			[unit.inputs, unit.outputs].forEach((arr) => {
				for (let i = 0; i < arr.length; i++) {
					const newVal = linkMap.has(arr[i])
						? linkMap.get(arr[i])
		...
		if (outerUnit) SimulationUnits.removePlugs(units, compiledComp);

		// udcIndexes is already sorted desc
		for (let i = udcIndexes.length - 1; i >= 0; i--) {
			const index = udcIndexes[i];
			const innerIdIdentifier = `${idIdentifier}:${
				[...compiledComp.units.values()][udcIndexesInclPlug[i]].id
			}`;
			const inner = this.projectUnits(
				units[index].type,
				innerIdIdentifier,
				units[index]
			);
	...
	private pushWiresOnLink(
		...
		if (
			this._wiresOnLinks.get(idIdentifier).has(newVal) &&
			this._wiresOnLinks.get(idIdentifier).get(newVal).length > 0
		) {
			if (
				this._wiresOnLinks.get(idIdentifier).get(newVal)[0] !==
				this._wiresOnLinksCache.get(typeIdentifier).get(val)[0]
			) {
				this._wiresOnLinks
					.get(idIdentifier)
					.get(newVal)
					.push(
						...(this._wiresOnLinksCache.get(typeIdentifier).get(val) || [])
					);
			}
		} else {
			this._wiresOnLinks
		...
		if (
			this._wireEndsOnLinks.get(idIdentifier).has(newVal) &&
			this._wireEndsOnLinks.get(idIdentifier).get(newVal).length > 0
		) {
			if (
				this._wireEndsOnLinks.get(idIdentifier).get(newVal)[0] !==
				this._wireEndsOnLinksCache.get(typeIdentifier).get(val)[0]
			) {
				this._wireEndsOnLinks
					.get(idIdentifier)
					.get(newVal)
					.push(
						...(this._wireEndsOnLinksCache.get(typeIdentifier).get(val) || [])
					);
			}
		} else {
			this._wireEndsOnLinks
	...
	private setIOLink(
		...
		compiledComp: CompiledComp,
		unitIndex: number,
		realUnit: SimulationUnit
	) {
		if (!this._ioElemUnitCache.has(idIdentifier)) {
			this._ioElemUnitCache.set(
				idIdentifier,
				new Map<ElementId, SimulationUnit>()
			);
		}
		...
		for (const element of project.currState.allPlugs()) {
			if (plugIndices.has(element.plugIndex)) {
				...
				throw new InvalidPlugsError(
					isInput
						? 'ERROR.COMPILE.INVALID_INPUT_PLUGS'
						: 'ERROR.COMPILE.INVALID_OUTPUT_PLUGS',
					project.id,
					(isInput
						? element.plugIndex
						: element.plugIndex + project.numInputs) + 1
				);
			}

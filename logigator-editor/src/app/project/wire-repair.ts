...
/**
 * `kind` mirrors the invariants in `docs/wires.md`: overlapping collinear wires
 * (a state even I1–I3 assume away), an endpoint or port inside a wire's
 * interior (I1/I2), and a collinear pair touching without a third terminator
 * (I3).
 */
...
export interface WireRepairPlan {
  /** Live tree wires whose geometry the rebuild changed. */
  removeWires: Wire[];
  /** Fresh replacement instances, never in the tree. */
  addWires: Wire[];
}

/**
 * Scans the whole board for wire-invariant violations. A pure read that does
 * not judge how the state arose, so the caller decides whether a live scissor
 * cut — a deliberate, transient I3 violation — is in play.
 *
 * Runs off a {@link WireRowColumnIndex} instead of the quad tree and issues no
 * rect query: collinear pairs are a sorted sweep per grid line, a buried
 * endpoint or port a lookup of the two lines through it. Both are bounded by
 * what shares a line, so the scan is independent of how long the wires are.
 */
export function auditWireInvariants(project: Project): WireViolation[] {
  ...
  // Overlap and I3, one line sweep per bucket: in axis order a wire only meets
  // those starting before it ends, so the inner scan can stop there.
  for (const sorted of index.lines()) {
    for (let i = 0; i < sorted.length; i++) {
      ...
      for (let j = i + 1; j < sorted.length; j++) {
        ...
        // The bucket key is a floored coordinate, so an off-lattice wire can
        // share a line without being collinear.
  ...
  // I1/I2: an endpoint or port buried in a wire's interior. Such a point sits
  // on its wire's centre line, so only the row and column through it can hold
  // one — a point lookup, never a span query.
  for (const a of wires) {
    for (const p of a.connectionPoints) {
      index.forEachInteriorContaining(p, (other) => {
        // Collinear neighbours, `a` included, are the sweep's business.
...
/**
 * The minimal wire diff restoring every invariant, computed without touching
 * the project. Rebuilds the board in an offline working set, each wire re-added
 * through the integrator against those already placed, so the integrator only
 * ever sees valid state plus one wire — its designed use case. Wires whose
 * geometry survives are matched back to their originals and stay untouched,
 * keeping their ids, selection and history references.
 */
export function computeWireRepair(project: Project): WireRepairPlan {
  ...
  // Zero-length wires carry no span and no valid endpoints.
